...
def fast_gknn(dist_matrix, directed, k):
    """
    ...
    knn = np.zeros((N, k, 2), dtype=DTYPE, order='C')
    dijkstra(dist_matrix, knn, directed, k)

    return knn

@cython.boundscheck(False)
cdef np.ndarray dijkstra(dist_matrix,
                         np.ndarray[DTYPE_t, ndim=3] knn,
                         int directed=0,
                         int k=10):
    """
    ...
    """
    cdef unsigned int N = knn.shape[0]
    cdef unsigned int i
...
cdef void dijkstra_directed_one_row(
                    ...
                    np.ndarray[DTYPE_t, ndim=3, mode='c'] knn,
                    FibonacciHeap* heap,
                    FibonacciNode* nodes,
                    int k):
    """
